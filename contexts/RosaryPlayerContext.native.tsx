  ...
  useEffect(() => {
    ...
    const restoreState = async () => {
      try {
        ...
        if (!savedState) {
          if (__DEV__) console.log('[RosaryPlayerContext] No saved rosary state found');
          return;
        }
        
        if (__DEV__) {
          console.log('[RosaryPlayerContext] Restoring saved rosary state:', {
        ...
        if (__DEV__) {
          console.log('[RosaryPlayerContext] Rosary state restored successfully');
        }
      } catch (error) {
        console.error('[RosaryPlayerContext] Error restoring rosary state:', error);
      }
    };
    ...
    saveStateDebounceRef.current = setTimeout(() => {
      if (currentMystery && rosaryForm && beads.length > 0) {
        ...
        saveRosaryState(state).catch(err => 
          console.error('[RosaryPlayerContext] Error saving state:', err)
        );
      }
  ...
  useEffect(() => {
    if (shouldInitializeOnly && beads.length > 0) {
      const initializeOnly = async () => {
        console.log('[RosaryPlayerContext] Initializing queue for restoration with', beads.length, 'beads (no auto-play)');
        await rosaryAudio.initializeQueue();
        
        // Skip to saved bead without playing
        const beadToRestore = lastKnownBeadIdRef.current;
        if (beadToRestore) {
          console.log('[RosaryPlayerContext] Restoring to bead:', beadToRestore);
