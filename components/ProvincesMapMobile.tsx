...
// Try to import react-native-maps, but handle the case where it might not be available
...
interface ProvincesMapMobileProps {
  onProvinceSelect?: (province: Province) => void;
}

export default function ProvincesMapMobile({ onProvinceSelect }: ProvincesMapMobileProps) {
  ...
  if (!MapView || !Marker || !Polygon) {
    return (
      <View style={[styles.container, styles.errorContainer]}>
        ...
        <Text style={[styles.errorText, { color: Colors[colorScheme ?? 'light'].textSecondary, fontSize: 14 }]}>
          Please install react-native-maps for mobile platforms
        </Text>
      </View>
    );
  }
